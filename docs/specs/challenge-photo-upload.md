# 챌린지 인증 사진 업로드/파일 저장 Spec

## 1. 목적

사용자가 수락한 챌린지를 인증 사진으로 완료하고 파일을 저장하는 기능 계약을 정의한다. 인증 사진 업로드 자체는 토큰 보상을 지급하지 않으며, 보상은 다른 사용자의 좋아요 3개 단위 milestone에서만 처리한다.

## 2. 담당 Owner

- Primary owner: 김찬혁
- Reviewer: 장태환
- Reviewer 지정 사유: 업로드 이후 보상은 좋아요 milestone 정책과 연결된다.
...
- 업로드 성공만으로는 토큰 잔액을 회복하지 않는다.
- 업로드 성공만으로는 `upload_reward` token transaction을 생성하지 않는다.
- 인증 사진 보상은 다른 사용자의 좋아요가 3, 6, 9개 등 3개 단위 milestone에 도달할 때만 지급한다.
...
- `challenge_photos`
  ...
  - 신규 업로드에서는 `upload_rewarded = false`를 유지한다.
...
- 사진 row 생성과 챌린지 완료 처리는 하나의 DB transaction으로 처리한다.
- 파일 저장 성공 후 DB transaction이 실패하면 저장된 파일 정리 정책을 service에 둔다.
- 토큰 보상 계산은 업로드 service에서 처리하지 않는다.
...
- 업로드 성공 시 토큰 잔액은 변경되지 않는다.
- 업로드 성공 시 `upload_reward` token transaction은 생성되지 않는다.
...
- 저장소 실패 시 DB에 사진 row와 token transaction이 남지 않는다.

## 10. Frontend 연동 참고사항

- `/challenges` 화면의 `active` 챌린지에서만 업로드 UI를 활성화한다.
- 업로드 전 클라이언트에서도 파일 크기와 형식을 1차 검증하되, 서버 검증 결과를 최종 기준으로 사용한다.
- 업로드 성공 후 챌린지 상태와 인증 사진 정보를 응답값으로 갱신한다.
- 토큰 잔액 갱신은 좋아요 milestone 보상 응답에서 처리한다.
...
- Clients should read `photo.file_url` for image rendering and `challenge.status` for completion state.
