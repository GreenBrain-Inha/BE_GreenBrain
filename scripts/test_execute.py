...
class TestLoadGuardrails:
    ...
    def test_loads_agents_md_when_present(self, executor, tmp_project):
...
class TestInvokeAgent:
    ...
    def test_invokes_codex_with_correct_args(self, executor, phase_dir):
        executor._executor = "codex"
    ...
    def test_output_json_records_executor(self, executor):
        executor._executor = "codex"
        mock_result = MagicMock(returncode=0, stdout="", stderr="")
        step = {"step": 2, "name": "ui"}
...
class TestMainCli:
    ...
    def test_invalid_executor_exits(self, tmp_project, phase_dir):
        with patch("sys.argv", ["execute.py", "0-mvp", "--executor", "gpt"]):
            with patch.object(ex, "ROOT", tmp_project):
                with pytest.raises(SystemExit):
                    ex.main()

    def test_executor_codex_accepted(self, tmp_project, phase_dir):
        with patch("sys.argv", ["execute.py", "0-mvp", "--executor", "codex"]):
            with patch.object(ex, "ROOT", tmp_project):
                inst = ex.StepExecutor("0-mvp", executor="codex")
                assert inst._executor == "codex"
